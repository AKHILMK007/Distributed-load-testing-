...
def test_config():
    ...
    if test_ty == "T":
        test_message= {
            "test_id":test_id,
            "test_type": test_ty,
            "test_message_delay": input_metrics,
...
def metrics():
    ...
    complete_metrics = {}
    my_dict = {key: [] for key in complete_metrics}


    for i in metrics_consume:
        j=i.value.decode('utf-8')
        if [] not in complete_metrics.values:
            min_time = min(k[2] for k in complete_metrics)
            max_time = max(k[3] for k in complete_metrics)
            sum_time = sum(k[0] for k in complete_metrics)
            tot_count = sum(k[1] for k in complete_metrics)
            median_time = statistics.median(k[5] for k in complete_metrics)
            complete_metrics = {key: [] for key in complete_metrics}
            print(min_time, max_time, sum_time/tot_count, median_time)
        complete_metrics[j[4]] = j
...
class HeartbeatReceiver:
    def _init_(self, topic, bootstrap_servers, heartbeat_timeout=10):
