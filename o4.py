...
def metrics():
    ...
    bootstrap_servers1 = 'localhost:9092'
    topic1 = 'metrics_final'
    produce_to_server = KafkaProducer(bootstrap_servers=bootstrap_servers1)
    for i in metrics_consume:
        j=i.value.decode('utf-8')
        data_to_send = j
        produce_to_server.send(topic1, value=data_to_send.encode('utf-8'))
        produce_to_server.flush()
        print(j)
