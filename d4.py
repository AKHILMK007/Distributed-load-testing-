    ...
    for message in consumer:
        ...
        with config_lock:
            if config_data['test_type'] == 'A':
                ...
                tsunami1 = threading.Thread(target=tsunami, args=(config_data,))
                heartbeat_1=threading.Thread(target=generate_heartbeat_code , args=())
                heartbeat_1.start()
                tsunami1.start()
                tsunami1.join()
...
def avalanche(config_data):
    # global config_data
    global driver_node_id
    log=open('log.txt','a')
    log.write(str(config_data))
    driver = KafkaProducer(bootstrap_servers=['localhost:9092'])
    delay = 1/int(config_data['throughput'])
    response_times = []
    mean_time = [0, 0]
    result = []
    ...
    for i in range(message_count_per_driver):
        ...
        mean_time[1] += 1
        print(response)
        log.write(f"{driver_node_id}, {start_time}, {end_time}, {response_time}, {response.status_code}"+"\n")
...
def tsunami(config_data):
    ...
    for i in range(int(config_data["message_count_per_driver"])):
        ...
        response = requests.get("http://127.0.0.1:8000/ping")
        end_time = time.time()
        time.sleep(int(config_data["test_message_delay"]))
...
def generate_heartbeat_code():
    ...
    heartbeat_code = driver_node_id.encode('utf-8')
    while True:
            producer.send("heartbeat", value=heartbeat_code)
            time.sleep(3)
